...
def create_project_document_report(project_data, project_info, weekly_hours_data, title):
    ...
    if not project_data.empty:
        ...
        for emp_code in unique_employees:
            ...
            allocation_columns = ['effective_from', 'effective_to', 'allocation_percentage', 'allocation_status',
                                  'change_reason']
    ...
    if not weekly_hours_data.empty:
        ...
        for emp_code in unique_hour_employees:
            ...
            emp_hours['week_start'] = emp_hours['work_date'] - pd.to_timedelta(emp_hours['work_date'].dt.dayofweek,
                                                                               unit='d')
            ...
            for week_start, week_data in weekly_summary.iterrows():
                ...
                avg_hours_per_day = week_data['hours_worked'] / week_data['days_worked'] if week_data[
                                                                                                'days_worked'] > 0 else 0
...
def show_project_master_report(engine=None, db_pool=None):
    ...
    try:
        projects_df = run_query(
            "SELECT project_id, project_name, client_name, status FROM project ORDER BY project_name", engine, db_pool)

        if not projects_df.empty:
            project_options = ["Select a Project"] + [f"{row['project_name']} ({row['project_id']})" for _, row in
                                                      projects_df.iterrows()]
            ...
            if selected_project != "Select a Project":
                ...
                with col1:
                    start_date = st.date_input("📅 From Date", value=datetime.now() - timedelta(days=90),
                                               key="proj_start_date")
                with col2:
                ...
                if not project_data.empty:
                    ...
                    with col2:
                        avg_allocation = project_data[project_data['allocation_status'] == 'Active'][
                    ...
                    if len(current_employees) > 0:
                        ...
                        for emp_code in current_employees:
                            ...
                            current_alloc_pct = current_allocation['allocation_percentage'].iloc[
                                0] if not current_allocation.empty else 0

                            with st.expander(
                                    f"👤 {emp_info['employee_name']} ({emp_code}) - **{current_alloc_pct}% allocated**"):
                                    ...
                                    **Allocation History:**
                                    ...
                                    for _, allocation in emp_data.iterrows():
                                        ...
                                        if pd.notna(allocation['effective_to']):
                                            duration_text += f" to {allocation['effective_to']}"
                                            days = (pd.to_datetime(allocation['effective_to']) - pd.to_datetime(
                                                allocation['effective_from'])).days
                                            duration_text += f" ({days} days)"
                                            status_icon = "🔴"
                                        else:
                                            duration_text += " (Current)"
                                            days = (datetime.now().date() - pd.to_datetime(
                                                allocation['effective_from']).date()).days
                                        ...
                                        if allocation.get('change_reason'):
                                            st.markdown(
                                                f"&nbsp;&nbsp;&nbsp;&nbsp;**Reason:** {allocation['change_reason']}")
                                        st.markdown("")
                    ...
                    if len(past_employees) > 0:
                        ...
                        for emp_code in past_employees:
                            ...
                            with st.expander(
                                    f"👤 {emp_info['employee_name']} ({emp_code}) - **Worked {total_days} days**"):
                                col1, col2 = st.columns([2, 1])
                    ...
                    if not weekly_hours_data.empty:
                        ...
                        weekly_chart_data['week_start'] = pd.to_datetime(
                            weekly_chart_data['work_date']) - pd.to_timedelta(
                            pd.to_datetime(weekly_chart_data['work_date']).dt.dayofweek, unit='d')
                        weekly_totals = weekly_chart_data.groupby(['week_start', 'employee_name'])[
                        ...
                        fig = px.bar(weekly_totals, x='week_start', y='hours_worked', color='employee_name',
                                     ...
                                     labels={'week_start': 'Week', 'hours_worked': 'Hours'})
